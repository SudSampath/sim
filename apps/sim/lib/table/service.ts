...
export async function createTable(
  ...
  // this, a table could be created directly inside a locked folder.
  if (data.folderId) {
    await assertFolderMutable(data.folderId, 'table')
  }
...
export async function restoreTable(tableId: string, requestId: string): Promise<void> {
  ...
  // `locked` flag and its (unchanged) containing folder chain -- without it, a table
  // could be restored back into a folder that was locked after it was deleted.
  await assertResourceMutable('table', tableId)
